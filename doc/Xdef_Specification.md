...
XML Schemaの主要な機能を、Xdefで表現できます。
Xdefのファイル形式は、`.xdef`です。
エンコーディング形式は、**UTF-8のみ**が許されています。
...
`Child`は`Root`の子要素です。
このようにインデントを使用することで、ノードの階層を表現できます。
...
#### パーティクルの指定
複雑型要素の宣言では、`::`を使用することでパーティクルを指定できます。
パーティクルの指定をすることで、子ノードの出現や順番を指定できます。
...
例えば、下記はパーティクルの指定として、`choice`を使用しています。
...
## 型
ノードの宣言において、`:`で型を指定できます。
XML Schemaで定義済みの型(単純型)が、そのまま使用できることに加え、制限型というrestrictionを使用するような型を表現できます。
下記は、それぞれの使用方法を説明します。
...
## ノードジェネレータ
通常のノードの宣言では任意の要素を許容するanyを宣言できません。
